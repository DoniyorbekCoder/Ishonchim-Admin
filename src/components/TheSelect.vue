<script lang="ts" setup>
  import { getSelections_API } from '@/services/common';
...
const data = reactive<{offset: number, searchOffset:number, loading: boolean, count:number, list: any[], visible: boolean, value?: Tag, query:  string}>({
  offset: 0,
  searchOffset: 0,
  loading: false,
  count: 0,
...
<template>
  <ul class="menu">
    ...
    <ul :style="{ height: data.list.length + 'px', maxHeight: '300px'}" class="dropdown-menu dropdown_menu--animated dropdown_menu-6 bg-gray-primary" ref="scrollableElement" :class="data.visible ? 'block': 'none'" @scroll="handleScroll">
      <input type="search" placeholder="Qidiruv"  class="search px-4 focus:outline-none" v-model="data.query" @input="loadSelections" @keypress.enter.prevent="null">
